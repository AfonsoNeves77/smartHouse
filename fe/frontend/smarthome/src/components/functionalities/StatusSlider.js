...
const StatusSlider = ({ initialStatus, onUpdate, actuatorId }) => {
    ...
    const handleSetStatus = () => {
        ...
        console.log(`Sending request to /api/smarthome/actuators/${actuatorId}/act with command: ${newStatus}`);

        // Make a POST request to update the status
        fetch('/api/smarthome/actuators/${actuatorId}/act?command=${newStatus}', {
            method: 'PATCH',
            headers: {
